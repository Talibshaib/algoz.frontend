...
export default function DirectLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    setMessage('Attempting login on all available endpoints...');
    ...
    for (const endpoint of endpoints) {
      try {
        ...
        if (response.status === 200 && response.data.data) {
          ...
          setMessage(`Login successful via ${endpoint}! Redirecting...`);
        ...
        console.error(`Login failed on ${endpoint}:`, error);
        // Continue to next endpoint
      }
    }
    
    // If we get here, all endpoints failed
    setMessage('Login failed on all endpoints. Please check your credentials and try again.');
    setLoading(false);
  };
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <p className="mt-2 text-center text-sm text-gray-600">
          This page will try all available endpoints to log you in
        </p>
      </div>
